...
MQTTHandler::MQTTHandler(boost::asio::io_context& ioc)
    : m_ioc(ioc), m_client(std::make_unique<client_t>(ioc.get_executor())), m_running(true), m_connected(false) {
                ...
                try {
                    ...
                    auto connack_opt = co_await m_client->async_start(
                        am::v5::connect_packet{
                            true, 0x1234, "", std::nullopt, username, password
                        },
                        as::use_awaitable
                    );
                    ...
                    as::co_spawn(
                        ...
                        [this]() -> as::awaitable<void> {
                            try {
                                ...
                                std::cerr << "MQTT receive loop error: " << e.what() << std::endl;
                            }
                            co_return;
                        },
                        as::detached
