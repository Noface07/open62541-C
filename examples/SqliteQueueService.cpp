...
SqliteQueueService::SqliteQueueService(const std::string& dbPath, const OfflineQueueOptions& options)
    : dbPath_(dbPath), options_(options), fullBacklogTable_("OfflineQueue"), latestValuesTable_("LatestValues"), db_(nullptr) {
    InitializeDatabase();
}
...
void SqliteQueueService::DisposeDB() {
    StopQueueWorker();
    StopApiUploadTimer();
    if (db_) {
        sqlite3_close(db_);
...
void SqliteQueueService::SetConfigurations(const std::map<int, long>& map) {
    datapointToOrgIdMap_ = map;
}

void SqliteQueueService::SetApiUrl(const std::string& url) {
    apiUrl_ = url;
}
...
void SqliteQueueService::SetMqttConnected(bool connected) {
    bool wasConnected = mqttConnected_.load();
    mqttConnected_.store(connected);
    std::cout << "[SQLite] MQTT connection state changed: " << (connected ? "CONNECTED" : "DISCONNECTED") << std::endl;
    
    // If we just connected and weren't connected before, wake up the API upload worker
    if (connected && !wasConnected) {
        std::cout << "[SQLite] MQTT reconnected - notifying API upload worker" << std::endl;
    }
}

bool SqliteQueueService::IsMqttConnected() {
    return mqttConnected_.load();
}

void SqliteQueueService::TriggerApiUpload() {
    std::cout << "[SQLite] TriggerApiUpload called - checking worker status..." << std::endl;
    
    // Check if the API upload worker is running
    if (!apiUploadWorkerRunning_.load()) {
        std::cout << "[SQLite] API upload worker is not running - starting it now!" << std::endl;
        StartApiUploadTimer();
    } else {
        std::cout << "[SQLite] API upload worker is already running - notifying it" << std::endl;
...
void SqliteQueueService::InitializeDatabase() {
    ...
    const char* createLatest =
        "CREATE TABLE IF NOT EXISTS LatestValues ("
...
void SqliteQueueService::EnqueueMessage(const std::string& topic, const MqttPayload& payload, long orgId) {
    {
...
void SqliteQueueService::StartQueueWorker() {
    if (queueWorkerRunning_) return;
    queueWorkerRunning_ = true;
    workerThread_ = std::thread(&SqliteQueueService::QueueWorkerLoop, this);
}

void SqliteQueueService::StopQueueWorker() {
    if (!queueWorkerRunning_) return;
...
void SqliteQueueService::StartApiUploadTimer() {
    std::cout << "[SQLite] StartApiUploadTimer called - current worker status: " << (apiUploadWorkerRunning_ ? "RUNNING" : "STOPPED") << std::endl;
    
    if (apiUploadWorkerRunning_) {
        std::cout << "[SQLite] API upload worker already running - triggering notification" << std::endl;
        apiUploadCv_.notify_one();
        return;
    }
...
void SqliteQueueService::StopApiUploadTimer() {
    std::cout << "[SQLite] StopApiUploadTimer called - current worker status: " << (apiUploadWorkerRunning_ ? "RUNNING" : "STOPPED") << std::endl;
...
void SqliteQueueService::ApiUploadWorkerLoop() {
    while (apiUploadWorkerRunning_) {
        ...
        apiUploadCv_.wait(lock, [this] {
            ...
            bool shouldProceed = !apiUploadWorkerRunning_ || (isConnected && rowCount > 0);
            std::cout << "[SQLite] API upload worker check - MQTT: " << (isConnected ? "CONNECTED" : "DISCONNECTED") 
                      << ", DB rows: " << rowCount << ", shouldProceed: " << (shouldProceed ? "YES" : "NO") << std::endl;
            return shouldProceed;
        });

        if (!apiUploadWorkerRunning_) break;

        if (!SqliteQueueService::mqttConnected_.load() || GetRowCount() == 0) {
            std::cout << "[SQLite] Conditions not met for API upload, going back to wait..." << std::endl;
        ...
        try {
            ...
            for (const auto& msg : messages) {
                ...
                int quality = 0;
                try { quality = std::stoi(payload.quality); } catch (...) {}

                apiTagDataList.push_back({
                    orgId, payload.tagId, payload.value, payload.tagType,
                    payload.timeStamp, payload.source, payload.datapointId,
            ...
            std::cout << "[SQLite] Attempting to send " << apiTagDataList.size() << " records to API..." << std::endl;
            
            if (SendToApi(finalPayload.dump())) {
                std::vector<int> idsToDelete;
                for (const auto& msg : messages) idsToDelete.push_back(msg.id);
                DeleteMessages(idsToDelete);
                std::cout << "[SQLite] Successfully uploaded " << idsToDelete.size() << " messages to API" << std::endl;
            } else {
                std::cerr << "[SQLite] API upload failed, will retry later" << std::endl;
                std::cout << "[SQLite] Sleeping for 5 seconds before retry..." << std::endl;
                std::this_thread::sleep_for(std::chrono::seconds(5)); // backoff
                std::cout << "[SQLite] Woke up from backoff, continuing upload loop..." << std::endl;
            }
        } catch (const std::exception& ex) {
            std::cerr << "[SQLite] ApiUploadWorkerLoop exception: " << ex.what() << std::endl;
        }
...
void SqliteQueueService::InsertMessageToDatabase(const QueueItem& item) {
    ExecuteDbCommand([&](sqlite3* db) {
        ...
        std::cout << "[SQLite] InsertMessageToDatabase - Topic: " << item.topic << ", Payload size: " << payloadJson.size() << std::endl;

        // 1. Insert or Replace into LatestValues
        sqlite3_stmt* stmt1;
        const char* sql1 = "INSERT OR REPLACE INTO LatestValues (Topic, Payload, CreatedAt) VALUES (?, ?, ?);";
        ...
        if (success) {
            sqlite3_stmt* stmt2;
            const char* sql2 = "INSERT INTO OfflineQueue (Topic, Payload, CreatedAt) VALUES (?, ?, ?);";
        ...
        if (success) {
            ...
            std::cout << "[SQLite] Successfully inserted message to both LatestValues and OfflineQueue tables" << std::endl;
        } else {
            sqlite3_exec(db, "ROLLBACK;", nullptr, nullptr, nullptr);
            std::cerr << "[SQLite] Failed to execute insert statements: " << std::string(sqlite3_errmsg(db)) << std::endl;
            throw std::runtime_error("Failed to execute insert statements: " + std::string(sqlite3_errmsg(db)));
        }
        return SQLITE_OK;
    });
}

std::vector<QueueModel> SqliteQueueService::GetBatchQueuedMessages(int batchSize) {
    std::vector<QueueModel> messages;
    ExecuteDbCommand([&](sqlite3* db) {
        sqlite3_stmt* stmt;
        const char* sql = "SELECT Id, Topic, Payload, CreatedAt FROM OfflineQueue ORDER BY Id ASC LIMIT ?;";
...
std::vector<QueueModel> SqliteQueueService::GetLatestValues() {
    std::vector<QueueModel> messages;
    ExecuteDbCommand([&](sqlite3* db) {
        sqlite3_stmt* stmt;
...
void SqliteQueueService::DeleteMessages(const std::vector<int>& ids) {
    if (ids.empty()) return;
    ExecuteDbCommand([&](sqlite3* db) {
...
void SqliteQueueService::ClearLatestValues() {
     ExecuteDbCommand([&](sqlite3* db) {
        char* errMsg = nullptr;
        if (sqlite3_exec(db, "DELETE FROM LatestValues;", nullptr, nullptr, &errMsg) != SQLITE_OK) {
             std::string err = errMsg;
             sqlite3_free(errMsg);
...
long SqliteQueueService::GetRowCount() {
    long count = 0;
    ExecuteDbCommand([&](sqlite3* db) {
        sqlite3_stmt* stmt;
...
void SqliteQueueService::PublishLatestValuesToMqtt(const std::function<void(const std::string&, const std::string&)>& publishFunc) {
    std::cout << "[SQLite] PublishLatestValuesToMqtt called!" << std::endl;
    try {
        auto latestValues = GetLatestValues();
        std::cout << "[SQLite] Retrieved " << latestValues.size() << " latest values from database" << std::endl;
        ...
        std::cout << "[SQLite] Publishing " << latestValues.size() << " latest values to MQTT..." << std::endl;
        int errorCount = 0;
        for (const auto& value : latestValues) {
            try {
                std::cout << "[SQLite] Publishing latest value for topic: " << value.topic << std::endl;
                // The C# version wraps the payload in a {"Data": [...]} structure. Replicating that.
                nlohmann::json wrapper;
                wrapper["Data"] = nlohmann::json::array({ nlohmann::json::parse(value.payload) });
                std::string payloadToPublish = wrapper.dump();
                std::cout << "[SQLite] Calling publishFunc for topic: " << value.topic << " with payload size: " << payloadToPublish.size() << std::endl;
                publishFunc(value.topic, payloadToPublish);
                std::cout << "[SQLite] Successfully published latest value for topic: " << value.topic << std::endl;
            } catch (const std::exception& ex) {
                errorCount++;
                std::cerr << "[SQLite] Error publishing latest value for topic " << value.topic << ": " << ex.what() << std::endl;
            }
        }

        if (errorCount == 0) {
            ClearLatestValues();
            std::cout << "[SQLite] Cleared latest values table after successful MQTT publishing" << std::endl;
        } else {
            std::cerr << "[SQLITE] Keeping latest values table due to " << errorCount << " publishing errors" << std::endl;
        }

    } catch (const std::exception& ex) {
        std::cerr << "[SQLite] Unhandled exception in PublishLatestValuesToMqtt: " << ex.what() << std::endl;
    }
}
...
void SqliteQueueService::ExecuteDbCommand(const std::function<int(sqlite3*)>& command) {
    int maxRetries = 3;
    for (int i = 0; i < maxRetries; ++i) {
        ...
        if (rc == SQLITE_BUSY || rc == SQLITE_LOCKED) {
            std::cerr << "Database is locked. Retrying attempt " << (i + 1) << "/" << maxRetries << "..." << std::endl;
            std::this_thread::sleep_for(std::chrono::milliseconds(100));
        } else {
            throw std::runtime_error("SQLite operation failed: " + std::string(sqlite3_errmsg(db_)));
        }
    }
    throw std::runtime_error("SQLite operation failed after multiple retries: database remained locked.");
}


std::string SqliteQueueService::GetCurrentTimestamp() {
